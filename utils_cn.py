...
def format_player_list(players: List[PlayerLike], show_roles: bool = False) -> str:
    """格式化玩家列表为中文显示"""
    if not players:
        return "无玩家"
...
def check_winning_cn(alive_players: List[PlayerLike], roles: Dict[str, str]) -> Optional[str]:
    """检查中文版游戏胜利条件"""
